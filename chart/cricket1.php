...
//get data for chart
$connector->query("select year,month,day,cricket from record order by year desc,month desc,day desc limit 0,20");
$r_date=array();
$r_score=array();
while(list($year,$month,$day,$cricket)=mysql_fetch_row($connector->result)){
	array_push($r_date,$month."/".$day);
	array_push($r_score,$cricket);
}
