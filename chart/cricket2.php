...
for($i=0;$i<count($year);$i++){
	$connector->query("select cricket from record where year=$year[$i] and month=$month[$i] and day=$day[$i]");
	list($cricket)=mysql_fetch_row($connector->result);
	array_push($date,$year[$i]."/".$month[$i]);
	array_push($score,$cricket);
}
