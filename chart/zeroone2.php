...
for($i=0;$i<count($year);$i++){
	$connector->query("select zeroone from record where year=$year[$i] and month=$month[$i] and day=$day[$i]");
	list($zeroone)=mysql_fetch_row($connector->result);
	array_push($date,$year[$i]."/".$month[$i]);
	array_push($score,$zeroone);
}
...
//adjust color
$serieSettings = array("R"=>0,"G"=>0,"B"=>255);
